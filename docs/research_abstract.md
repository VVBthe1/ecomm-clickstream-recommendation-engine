...
Expected outcomes are: a reproducible pipeline (`make all`, `make train`, `make tune`, `make api`), documented gold feature tables, comparative metrics across temporal experiments (E1 in-month, E2 early-November generalization, E_final Oct–mid-Nov train / late-Nov test), saved model artifacts, SHAP-based feature explanation for the best tree model, and a demo FastAPI predict endpoint. The contribution is a practical, documented link between open clickstream data and product-level demand forecasting — not a new algorithm, but an implementable system with explicit preprocessing, feature definitions, and evaluation against established baselines and peer-reviewed comparison practice.
